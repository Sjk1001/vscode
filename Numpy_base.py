...
# 0以上1未満のランダムな値を一つ生成し、aに代入してください。
a = np.random.rand()

print(a)
print('-----------------------------------------------------------------------------')

########5.1.2########## 5.1.2 ランダムサンプリング
import numpy as np


np.random.seed(0)

# 平均0・分散1の正規分布に従う乱数を、3行2列の2次元配列として6つ生成し、arr_1に代入してください。
arr_1 = np.random.randn(3,2)

print(arr_1)
print('-----------------------------------------------------------------------------')

########5.2.1########## numpy.where()関数
import numpy as np
...
arr_2 = np.where(arr_1 == 6, True, False)
print(arr_2)
